...
BERTScore decays approximately 3$\times$ slower than BLEU-2.}
...
We train a 4-class Random Forest classifier on T1 data to determine whether
...
\midrule
\textbf{Overall} & \multicolumn{2}{l}{\textbf{Acc: 0.583}} & \textbf{0.41} & 875 \\
\bottomrule
\end{tabular}
\captionof{table}{Paraphraser fingerprint classifier performance (4-class).
Dipper is most identifiable (F1 = 0.69); PaLM is nearly indistinguishable
from other families (F1 = 0.02).}
...
\textbf{Paraphrasers leave partially identifiable fingerprints.} Overall
accuracy of 58.3\% significantly exceeds the 25\% chance baseline,
confirming that paraphrasers do leave stylistic signatures. However,
performance is highly uneven across families.

\textbf{Dipper is the most distinctive paraphraser}, achieving 0.90 recall
and 0.69 F1. Its discourse-level rewriting strategy produces
characteristically different output from the other models. Pegasus is
moderately identifiable (F1 = 0.58), while ChatGPT (F1 = 0.35) and
especially PaLM (F1 = 0.02) are difficult to distinguish. PaLM's near-zero
recall suggests it produces output stylistically similar to ChatGPT, as
confirmed by the confusion matrix.

\textbf{Stylometric features dominate fingerprinting.} The top features by
importance are \texttt{pos\_PUNCT} (0.022), \texttt{pos\_CCONJ} (0.019),
and \texttt{sent\_len\_var} (0.018). This indicates that paraphrasers
differ most in their treatment of punctuation, coordination patterns, and
sentence-level structure rather than in vocabulary choice.
